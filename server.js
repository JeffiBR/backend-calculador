...
// Função para garantir que a tabela products existe (corrigida para o nome correto)
async function garantirTabelaProducts() {
    try {
        ...
        if (checkError && checkError.code === '42P01') {
            // Tabela não existe, criar com estrutura corrigida
            console.log('Criando tabela products (estrutura compatível com a calculadora)...');
            ...
            const { error: createError } = await supabase.rpc('exec_sql', {
                sql: `
                    CREATE TABLE products (
                        ...
                        -- Campos adicionais da calculadora para compatibilidade
...
app.post('/api/products', uploadProduto.single('produtoFoto'), async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            // Em ambiente de produção, faça upload para Supabase Storage
...
// Rota para criar tabela de products
...
app.listen(port, async () => {
    ...
    - GET  /api/criar-tabela-products - Criar tabela de products
    ...
    - POST /api/products             - Salvar produto (calculadora)
    ...
    try {
        ...
        await garantirTabelaProducts();
        console.log('✅ Tabela de products verificada/criada com sucesso!');
    } catch (error) {
